<x-jet-dropdown align="right" width="96">
    <x-slot name="trigger">
        <div x-data="{showUnreadNotifications : true}">
            <button
                class="relative flex p-1 text-sm transition border-2 rounded-full focus:outline-none focus:border-gray-300"
                ...
                @if ($totalUnreadNotifications)
                    <div x-show="showUnreadNotifications"
                        class="absolute flex items-center justify-center w-6 h-6 text-xs font-bold transform scale-75 bg-orange-300 rounded-full -right-1 -top-2">
    ...
    <x-slot name="content">
        <ul class="w-full max-w-full pb-2 overflow-y-auto divide-y max-h-96 sm:w-96" id="notif-container">
            @forelse ($notif as $n)
                <x-jet-dropdown-link href="{{ $n->data['link'] }}"
                    class="{{ $n->read_at ? __('bg-white') : __('bg-gray-100') }} ">
                    <p class="text-xs">{!! $n->data['message'] !!}</p>
                    <p class="my-1 text-xs italic text-right text-gray-500">
                        {{ $n->created_at->format('d M Y H:i') }} WIB
                    </p>
                </x-jet-dropdown-link>
            @empty
                <li class="p-2 text-sm text-gray-500">
                    <p class="italic text-center">Belum ada notifikasi</p>
                </li>
            @endforelse
        </ul>
    </x-slot>
