@props(['description', 'isLeft', 'isRead', 'createdAt', 'canDelete' => false])

<li class="flex mb-3 {{ $isLeft ? 'ml-3' : 'mr-3 justify-end' }}">
    <div
        class="p-4 rounded-b-lg w-auto max-w-3xl relative {{ $isLeft ? 'bg-light-4 rounded-tr-lg' : 'bg-subtle rounded-tl-lg' }}">
        ...
        <p class="text-xs italic mt-1 w-full flex {{ $canDelete ? 'justify-between' : 'justify-end' }}">
            @if (!$isLeft && $canDelete)
                <span class="text-red-700 font-bold cursor-pointer mr-4">Delete</span>
            @endif
