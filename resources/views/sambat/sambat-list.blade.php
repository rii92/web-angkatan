    ...
    <div class="mx-auto">
        ...
        <div class="flex flex-col lg:grid lg:grid-cols-3 gap-4 lg:mb-12 lg:mx-28">
            ...
            <a href="{{ route('user.sambat.add') }}"
                class="w-full p-2 text-base font-medium text-center text-white duration-300 bg-sky-900 hover:-translate-y-1 hover:scale-105 rounded-xl drop-shadow-2xl">
                Buat Sambatanmu
            </a>
        </div>
                ...
                @forelse ( $sambat as $s )
                    <div class="mx-24 my-7">
                        <div class="flex flex-row mb-2">
                            <div class="mr-1">
                                <a href="{{ url('sambat/user/' . $s->user_id) }}"><img
                                        class="object-cover w-full rounded-full"
                                        src="{{ $s->users->profile_photo_url }}"
                                        alt="{{ $s->is_anonim ? 'Anonim' : $s->users->name }}" /></a>
                            </div>
                            <div>
                                <p class="text-base text-gray-500"><a href="{{ url('sambat/user/' . $s->user_id) }}"
                                        class="mr-2">{{ $s->is_anonim ? 'Anonim' : $s->users->name }}</a><span
                                        class="mr-2">• Kelas </span><span class="mr-2">•
                                        {{ $s->created_at }}</span></p>
                                <div class="mt-1">
                                    @foreach ($s->tags as $t)
                                        <a href="{{ url('sambat/tag/' . $t->id) }}"
                                            class="p-2 bg-gray-200 rounded-xl hover:bg-gray-300 m-1">{{ $t->name }}</a>
                                    @endforeach
                                </div>
                            </div>
                        </div>
